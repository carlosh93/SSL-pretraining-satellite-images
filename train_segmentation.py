...
def main(cfg):
    ...
    task = SemanticSegmentationTask(
        ...
        loss=cfg.segm_loss,
        weights='ssl',  # ssl weights on imagenet, will train with this if train-mode is imagenet-pretrain
    ...
    if cfg.train_mode == "SSL4EO-pretrain-finetune":
        # @Hanan, by default I will freeze all the encoder (ResNet backbone) by setting requires_grad to False to all
        # the parameters
        for name, param in task.model.encoder.named_parameters():
            param.requires_grad = False
        # @Hanan, however you can play by freezing the parameters you want, for example:
        # first, define which layers to freeze by looking at the name of the parameters:
        # for name, param in task.model.encoder.named_parameters():
        #     print(name)
        ...
        #         param.requires_grad = False
        # you can check if a particular parameter is frozen by using, for example:
        # task.model.encoder.layer4[0].conv1.weight.requires_grad
    ...
    wandb_logger = WandbLogger(project="SSL-Pretraining-Satellite-Images", save_dir=experiment_dir, log_model="all")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ew", "--encoder-weights", type=str, default="ResNet50_Weights.SENTINEL2_RGB_MOCO",
                        help="Backbone Encoder Weights")
    ...
    parser.add_argument("-lrsp", "--learning-rate-schedule-patience", type=int, default=6, help="Learning Rate "
                                                                                                "Schedule Patience")
    ...
    parser.add_argument("-tm", "--train-mode", type=str, default="SSL4EO-pretrain",
                        choices=["imagenet-pretrain", "SSL4EO-pretrain", "SSL4EO-pretrain-finetune"], help="Train Mode")
